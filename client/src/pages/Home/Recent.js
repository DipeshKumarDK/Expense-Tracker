...
function Recent({ trans, thisMonth }) {
  return (
    <div className="bg-[#55575e] xl:w-2/5 lg:w-1/3 w-full lg:pt-16 pt-6 lg:pb-8 pb-4 lg:pl-2 lg:pr-4 pl-3 pr-3 lg:h-full rounded-md lg:ml-4">
      <h2 className="xl:text-3xl text-2xl text-slate-200 lg:mt-1">
        Recent Activities
      </h2>
      <section className="mt-3">
        {trans?.map((elem) => {
          return (
            <div
              className={`flex justify-between bg-[#2e2f34] ${
                elem?.type === "income" ? "text-green-400" : "text-red-400"
              } p-3 rounded-md mt-2 mb-2`}
            >
              <h3>{elem?.title}</h3>
              <h3>${elem?.amount}</h3>
            </div>
